...
async function initializeBrowser(config = {}) {
  ...
  try {
    ...
    if (process.env.GOOGLE_CHROME_BIN) {
      ...
      console.log('🔍 Production environment detected, using default Playwright browser...');
      // In production on Heroku with Playwright buildpack, browsers should be available
    }
